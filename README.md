...
- 支持 `google`、`baidu`、`youdao`、`tencent`、`openai`、`deepl` 多种 provider。
...
- `enabled`：是否启用划词翻译。
- `provider`：翻译服务，支持 `google`、`baidu`、`youdao`、`tencent`、`openai`、`deepl`。
...
使用百度翻译开放平台通用文本翻译 API。需要在百度翻译开放平台创建应用，并设置环境变量：

```powershell
[Environment]::SetEnvironmentVariable("BAIDU_TRANSLATE_APP_ID", "你的 APP ID", "User")
[Environment]::SetEnvironmentVariable("BAIDU_TRANSLATE_SECRET_KEY", "你的密钥", "User")
```
...
使用有道智云文本翻译 API。需要创建有道智云文本翻译应用，并设置环境变量：
...
使用腾讯云机器翻译 TextTranslate API。需要开通腾讯云机器翻译服务，并设置环境变量：
...
腾讯云接口的源语言自动识别能力可能随接口规则变化。如果 `source_language: "auto"` 被服务端拒绝，请在 `config.json` 中把 `source_language` 改成明确语言，例如 `en`、`ja`、`ko`。
...
- 如果使用 Google、百度、有道、腾讯云、DeepL 或 OpenAI 翻译，选中文本会发送给对应翻译服务。
- 软件本身不保存完整选中文本。
...
- 百度、有道、腾讯云 provider 需要用户自行开通对应云服务并承担对应平台可能产生的费用。
